...
for j in range(len(lowy_array)):
    ...
    # ax1.plot(area_array, capacity_array, linewidth=4, color="C%s"%(9-j))
    ax1.plot(area_array, coe_array, linewidth=4, color="C%s"%(9-j))

    # ax1.set_title("maximize capacity", fontsize=24)
    ax1.set_title("minimize COE", fontsize=24)
    ax1.set_xlabel("available area fraction", fontsize=24)
    ...
    plt.pause(0.001)
plt.savefig("figures/coe_v_area.png")
plt.show()
